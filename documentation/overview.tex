...
\title{Documentation of Assistance for Project nonFunc}
\author{Dennis Frank}
\date{}
\begin{document}
\maketitle
 {\fontfamily{lmss}\selectfont
\textbf{The documentation is still WORK IN PROGRESS, last updated on: 29.05.2026}\\
...
patchutils bc zlib1g-dev device-tree-compiler pkg-config libexpat-dev libfl-dev

sudo apt install libguestfs-tools 
\end{verbatim}
The following steps are neccesary to create a conda environment:\\
\begin{verbatim}
...
\includegraphics*[scale=0.8]{test6-2.jpg} 
\includegraphics*[scale=0.8]{image2.jpg}
\newpage
\subsubsection*{Workflow}
All of the code (the main code as well as the following code metioned) is located within the directory "chipyard/tests". 
Within this directory you can find the file "CMakeLists.txt". You need this file to add the follwing lines under the section "\#Build":
\begin{verbatim}
    add_executable(edgeBare edgeBare.c) 
    target_link_libraries(edgeBare m)
\end{verbatim}
Before the simulation it is neccesary to prepare the data of an image for the main code. Therefore you can use the code "image.py" by running:
\begin{verbatim}
    python3 image.py
\end{verbatim}
You have to enter an image file and the width and heiht of this file, right now it is only possible to use ".jpg" files. The code will write the image data to "image\_data.h" which is a two-dimensional array.
After this it is possible to compile the testfiles for the execution in the same directory with the following commands.
\begin{verbatim}
    cmake . 
